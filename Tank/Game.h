...
// Input the pos, and the function will tell you whether a tank is standing at the position,if it does,tank->id will be
// returned.
int Tankcheck(Vec pos) {
  for (RegIterator it = RegBegin(regTank); it != RegEnd(regTank); it = RegNext(it)) {
    Tank *tank = RegEntry(regTank, it);
    if (-1 <= Sub(pos, tank->pos).x && Sub(pos, tank->pos).x <= 1 && -1 <= Sub(pos, tank->pos).y &&
        Sub(pos, tank->pos).y <= 1) {
      return tank->id;
    }
  }
...
/// \brief This fuction is used to Create 3*3 something.
/// Since we need to add 3*3 solid and wall in the scene,which have the same process.
/// Give the things you want to creat as object,we can avoid the repeat code.
...
/// \brief This fuction is used to announce the winning,once all of the enermy was hit to 0 blood,the player win the
/// game,there are some sentence help player to know how to restart and congradulate for them.
void GameEndSuccess(void) {
  char *Dead1 = "You have pass the challenge!";
  char *Dead2 = "Game over";
  char *Dead3 = "Press 'R' to restart";
  char *Dead4 = "Press anything else to exit";
  int len = strlen(Dead1);
  int startposy = (map.size.y - 4) / 2;
  int startposx = (map.size.x - len) / 2;
  Vec pos;

  for (int y = startposy - 1; y < startposy + 5; ++y)
    for (int x = startposx - 2; x < startposx + len + 2; ++x) {
      ...
      Flag flag;
      if (x == startposx - 2 || y == startposy - 1 || x == startposx + len + 1 || y == startposy + 4) {
        flag = eFlagSolid;
  ...
  RdrPutString(startposy + 3, Dead1, TK_NORMAL);
  RdrPutString(startposy + 2, Dead2, TK_NORMAL);
  RdrPutString(startposy + 1, Dead3, TK_GREEN);
  RdrPutString(startposy, Dead4, TK_RED);
  game.alive = false;
}
...
void GameInput(void) {
  ...
  Tank *tank = RegEntry(regTank, it);
  if (game.state == Ready && (game.keyHit == 'w' || game.keyHit == 'a' || game.keyHit == 's' || game.keyHit == 'd')) {
    game.state = Normal;
  }
  ...
  if (game.alive == true) {
    if (game.keyHit == 'w') {
      ...
      renderer.Bloodprint = !renderer.Bloodprint; // To show or hide the blood each tank has.
    }
  } else if (game.alive == false && game.keyHit == 'r') {
    game.alive = true;
    GameInit();
...
void GameUpdate(void) {
  ...
  for (RegIterator it = RegBegin(regTank); it != RegEnd(regTank); it = RegNext(it)) {
    ...
    if (map.flags[Idx(newPosmiddle)] == eFlagNone && map.flags[Idx(newPosleft)] == eFlagNone &&
        map.flags[Idx(newPosright)] == eFlagNone && tank->isOperate && tank->id == 0 && Tankcheck(newPosmiddle) == -1 &&
        Tankcheck(newPosleft) == -1 && Tankcheck(newPosright) == -1) {
      tank->pos = newPos;
      tank->isOperate = false;
    }
    ...
    int shoot = config.EnemyPower == 0 ? 1 : Rand(40 / config.EnemyPower);
